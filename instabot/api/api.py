...
#        self.proxy = proxy
        ...
        if not use_cookie and (not self.is_logged_in or force):
            ...
            self.logger.info("your proxy" + proxy)
...
#                    sys.stdout.write('Checkpoint challenge required...')
#                    sys.stdout.flush()
                    self.logger.warning('Checkpoint challange required...')
                    with open("checkpoint.txt", "w+") as f:
                        f.write("True")
                    with open(ig_username + "check_blocked.txt", "r+") as f:
                        blocks = f.readlines()
                        for block in blocks:
                            block = str(block)
                            block = int(block)
...
# Nu valger 1 som standard pga stdin kun kan tage et input i run.py
        code = "1"
            ...
            try:
                ...
                if "feedback_required" in str(response_data.get('message')):
                    if response_data.get('feedback_title') == 'Action Blocked':
                        self.logger.error("This action was blocked. Going to sleep 60 seconds and relogin!")
                        time.sleep(60)
                        self.logout()
